...
treasuresprite=createSprite(canW-350,500)
...
gametitlesprite=createSprite(canW/2,canH/2-200)
gametitlesprite.addImage("gt",gametitle)
 
roofgroup=createGroup();
for(var r=1300;r<10000;r=r+600){
...
if(touches.length>0||keyDown("space")){
 ...
 if(ninja.y>700){
  ...
  if(ninja.isTouching(roofgroup)&&ninja.y>=canH-445) {
      var token=1;
...
for(x=0;x<coingroup.length;x++){
 ...
 if(distance>=1000){
   fill("black")
   textSize(40)
   text("LEVEL2 COMPLETED",ninja.x+300,canH/2-200)
   text("UNLOCK TREASURE",ninja.x+300,canH/2-130)
 // icon2.visible=true
ninja.velocityX=0;
ninja.velocityY=0;
//console.log(icon2.x)
